...
public class MyPrioritizedQueue<Item extends Comparable> {
    private Item[] list;
    private int size;
    private int DEFAULT_CAPACITY = 10;
    private Comparator<Item> comparator = Comparator.naturalOrder();

    public MyPrioritizedQueue(int capacity, Comparator comparator ) {
        if (capacity <= 0) {
    ...
    public MyPrioritizedQueue(int capacity) {
        if (capacity <= 0) {
            throw new IllegalArgumentException("bad capacity " + capacity);
        }
        list = (Item[]) new Comparable[capacity];
